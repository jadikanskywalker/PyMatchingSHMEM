...
#SBATCH --output=/mnt/DISCL/home/jadhicks/PyMatchingSHMEM/verify_tmp/out/verify_obs_preemptive_overlap_cascade-%j.out
...
python3 - "$DET" "$DEM" verify_tmp/out/det_zero_cascade.b8 <<'EOF'
import sys
import stim
...
oshrun -n 1 build_sos/pymatching predict --dem "$DEM" \
  --in verify_tmp/out/det_zero_cascade.b8 --in_format b8 \
  --out verify_tmp/out/pred_obs_preemptive_cascade.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 8 \
  --extract_preemptively \
  > verify_tmp/out/verify_obs_preemptive_cascade.stdout 2>&1
echo "exit=$?" >> verify_tmp/out/verify_obs_preemptive_cascade.stdout
