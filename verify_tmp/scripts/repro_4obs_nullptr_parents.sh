...
#SBATCH --output=/mnt/DISCL/home/jadhicks/PyMatchingSHMEM/verify_tmp/out/repro_4obs_nullptr_parents-%j.out
...
python3 dem_gen/scripts/gen_multi_obs.py \
    ...
    --num_surgery_gates 3 \
    --surgery_duration 5 \
    --circuit_out verify_tmp/out/repro4obs_circuit.stim \
    > verify_tmp/out/repro4obs_error_model.dem

python3 - verify_tmp/out/repro4obs_error_model.dem verify_tmp/out/repro4obs_det_zero.b8 <<'EOF'
import sys
import numpy as np
import stim
...
rm -rf out_parallel
oshrun -n 4 build_sos/pymatching predict --dem verify_tmp/out/repro4obs_error_model.dem \
  --in verify_tmp/out/repro4obs_det_zero.b8 --in_format b8 \
  --out verify_tmp/out/repro4obs_pred.01 --out_format 01 \
  ...
  > verify_tmp/out/repro_4obs_nullptr_parents.stdout 2>&1
echo "exit=$?" >> verify_tmp/out/repro_4obs_nullptr_parents.stdout
