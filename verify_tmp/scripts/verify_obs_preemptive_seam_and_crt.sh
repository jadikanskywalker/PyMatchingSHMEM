...
#SBATCH --output=/mnt/DISCL/home/jadhicks/PyMatchingSHMEM/verify_tmp/out/verify_obs_preemptive_seam_and_crt-%j.out
...
python3 - "$DET" "$DEM" verify_tmp/out/det_zero_9obs_3pe.b8 <<'EOF'
import sys
import stim
...
oshrun -n 3 build_sos/pymatching predict --dem "$DEM" \
  --in verify_tmp/out/det_zero_9obs_3pe.b8 --in_format b8 \
  --out verify_tmp/out/pred_obs_preemptive_seam_and_crt.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 1 \
  --extract_preemptively \
  > verify_tmp/out/verify_obs_preemptive_seam_and_crt.stdout 2>&1
echo "exit=$?" >> verify_tmp/out/verify_obs_preemptive_seam_and_crt.stdout
