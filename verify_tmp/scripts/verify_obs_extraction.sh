#!/bin/bash
#SBATCH --job-name=verify_obs_extraction
...
#SBATCH --output=/mnt/DISCL/home/jadhicks/PyMatchingSHMEM/verify_tmp/out/verify_obs_extraction-%j.out
...
FLIPS=~/PyMatchingSHMEM/testdems/actual_obs_flips_4obs_d21_p001_231r_250s.01

# Truncate to a handful of shots so the DEBUG=1 task-tree/trace output stays small and readable.
python3 - "$DET" "$DEM" verify_tmp/out/det_small.b8 <<'EOF'
import sys
import stim
...
oshrun -n 1 build_sos/pymatching predict --dem "$DEM" \
  --in verify_tmp/out/det_small.b8 --in_format b8 \
  --out verify_tmp/out/pred_obs_small.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 2 \
  > verify_tmp/out/verify_obs.stdout 2>&1
echo "exit=$?" >> verify_tmp/out/verify_obs.stdout
