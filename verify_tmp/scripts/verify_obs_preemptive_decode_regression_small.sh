...
python3 - "$DET" "$DEM" verify_tmp/out/det_small20.b8 <<'EOF'
import sys
import stim
...
oshrun -n 2 build_sos/pymatching predict --dem "$DEM" \
  --in verify_tmp/out/det_small20.b8 --in_format b8 \
  --out verify_tmp/out/pred_obs_nonpreemptive_small.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 1 \
  > verify_tmp/out/verify_obs_preemptive_decode_regression_small_nonpreemptive.stdout 2>&1
echo "exit=$?" >> verify_tmp/out/verify_obs_preemptive_decode_regression_small_nonpreemptive.stdout

echo "Preemptive OBS (Phase 2 under test)..."
oshrun -n 2 build_sos/pymatching predict --dem "$DEM" \
  --in verify_tmp/out/det_small20.b8 --in_format b8 \
  --out verify_tmp/out/pred_obs_preemptive_decode_small.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 1 \
  --extract_preemptively \
  > verify_tmp/out/verify_obs_preemptive_decode_regression_small_preemptive.stdout 2>&1
echo "exit=$?" >> verify_tmp/out/verify_obs_preemptive_decode_regression_small_preemptive.stdout

echo "Diffing predictions..."
if diff -q verify_tmp/out/pred_obs_nonpreemptive_small.01 verify_tmp/out/pred_obs_preemptive_decode_small.01 > /dev/null; then
    ...
    diff verify_tmp/out/pred_obs_nonpreemptive_small.01 verify_tmp/out/pred_obs_preemptive_decode_small.01 | head -20
fi
