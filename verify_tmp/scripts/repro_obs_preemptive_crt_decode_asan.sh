...
#SBATCH --output=/mnt/DISCL/home/jadhicks/PyMatchingSHMEM/verify_tmp/out/repro_obs_preemptive_crt_decode_asan-%j.out
...
python3 - "$DET" "$DEM" verify_tmp/out/det_small1.b8 <<'EOF'
import sys
import stim
...
oshrun -n 2 build_asan_throwaway/pymatching predict --dem "$DEM" \
  --in verify_tmp/out/det_small1.b8 --in_format b8 \
  --out verify_tmp/out/pred_obs_preemptive_decode_asan.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 1 \
  --extract_preemptively \
  > verify_tmp/out/repro_obs_preemptive_crt_decode_asan.stdout 2>&1
echo "exit=$?" >> verify_tmp/out/repro_obs_preemptive_crt_decode_asan.stdout
