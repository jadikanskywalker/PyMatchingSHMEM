...
void DecodingUnit::partition_detection_events(const std::vector<uint64_t>& detection_events) {
    hits.clear();
    for (auto det : detection_events) {
        int part_id = node_part_id[det];
        if (part_id >= 0) { // partition
            hits.partition_hits[part_id].push_back(det);
        } else {
            hits.virtual_boundary_hits[-1*part_id-1].push_back(det);
        }
    }
    if (DEBUG) {
        int i = 0;
        for (auto part : hits.partition_hits) {
        ...
        for (auto part : hits.virtual_boundary_hits) {
            std::cout << "VB" << i << " hits: ";
            for (int det : part) {
                std::cout << det << "  ";
            }
...
void DecodingUnit::solve_task(Task* task, int tid, int shot, int draw_frames) {
    task->setup();
    pm::Mwpm& solver = *solvers[tid];
    solver.prepare_for_task(task, shot);
    if (DEBUG && tid==0) {
        output_detector_nodes(solver, true);
    }
    auto& hitsref = (task->is_fusion) ? hits.virtual_boundary_hits[task->part] : hits.partition_hits[task->part];
    pm::process_timeline_until_completion(solver, hitsref, draw_frames, true, tid);
    if (DEBUG && tid==0) {
        output_solution_state(solver, hitsref, true);
    }
}