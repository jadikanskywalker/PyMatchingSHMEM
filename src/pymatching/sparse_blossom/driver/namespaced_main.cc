...
#ifdef USE_SHMEM
    // Each process outputs its own predictions contribution
    // XOR each process's predictions for overall prediction
        ...
        if (gen_code_arg != nullptr) {
...

            {
                ...
                if (preset_arg) {
                    ...
                    else if (preset == "144obs") surgeries = pm::MultiObsDemGenerator::preset_144obs();
                    else if (preset == "128obs") surgeries = pm::MultiObsDemGenerator::preset_128obs();
                    else if (preset == "256obs") surgeries = pm::MultiObsDemGenerator::preset_256obs();
...
#endif
        if (DEBUG) {
            pm::setup_output_dirs(
#ifdef ENABLE_DRAW_FLAGS
                draw_frames,
#endif
                config_parallel::use_threads
            );
        }
