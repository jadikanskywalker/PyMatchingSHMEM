...
#SBATCH --output=/mnt/DISCL/home/jadhicks/PyMatchingSHMEM/verify_out/verify_obs_preemptive_decode_regression-%j.out
...
oshrun -n 3 build_sos/pymatching predict --dem "$DEM" \
  --in "$DET" --in_format b8 \
  --out verify_out/pred_obs_nonpreemptive.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 1 \
  > verify_out/verify_obs_preemptive_decode_regression_nonpreemptive.stdout 2>&1
echo "exit=$?" >> verify_out/verify_obs_preemptive_decode_regression_nonpreemptive.stdout

echo "Preemptive OBS (Phase 2 under test)..."
oshrun -n 3 build_sos/pymatching predict --dem "$DEM" \
  --in "$DET" --in_format b8 \
  --out verify_out/pred_obs_preemptive_decode.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 1 \
  --extract_preemptively \
  > verify_out/verify_obs_preemptive_decode_regression_preemptive.stdout 2>&1
echo "exit=$?" >> verify_out/verify_obs_preemptive_decode_regression_preemptive.stdout

echo "Diffing predictions..."
if diff -q verify_out/pred_obs_nonpreemptive.01 verify_out/pred_obs_preemptive_decode.01 > /dev/null; then
    ...
    diff verify_out/pred_obs_nonpreemptive.01 verify_out/pred_obs_preemptive_decode.01 | head -20
fi
