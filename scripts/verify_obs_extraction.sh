...
#SBATCH --output=/mnt/DISCL/home/jadhicks/PyMatchingSHMEM/verify_out/verify_obs_extraction-%j.out
...
python3 - "$DET" "$DEM" verify_out/det_small.b8 <<'EOF'
import sys
import stim
...
oshrun -n 1 build_sos/pymatching predict --dem "$DEM" \
  --in verify_out/det_small.b8 --in_format b8 \
  --out verify_out/pred_obs_small.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 2 \
  > verify_out/verify_obs.stdout 2>&1
echo "exit=$?" >> verify_out/verify_obs.stdout
