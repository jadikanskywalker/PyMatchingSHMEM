...
python3 - "$DET" "$DEM" verify_out/det_small1_v2.b8 <<'EOF'
import sys
import stim
...
oshrun -n 2 build_sos/pymatching predict --dem "$DEM" \
  --in verify_out/det_small1_v2.b8 --in_format b8 \
  --out verify_out/pred_obs_nonpreemptive_1shot.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 1 \
  > verify_out/verify_obs_preemptive_decode_regression_1shot_nonpreemptive.stdout 2>&1
echo "exit=$?" >> verify_out/verify_obs_preemptive_decode_regression_1shot_nonpreemptive.stdout

echo "Preemptive OBS (Phase 2 under test)..."
oshrun -n 2 build_sos/pymatching predict --dem "$DEM" \
  --in verify_out/det_small1_v2.b8 --in_format b8 \
  --out verify_out/pred_obs_preemptive_decode_1shot.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 1 \
  --extract_preemptively \
  > verify_out/verify_obs_preemptive_decode_regression_1shot_preemptive.stdout 2>&1
echo "exit=$?" >> verify_out/verify_obs_preemptive_decode_regression_1shot_preemptive.stdout

echo "Diffing predictions..."
if diff -q verify_out/pred_obs_nonpreemptive_1shot.01 verify_out/pred_obs_preemptive_decode_1shot.01 > /dev/null; then
    ...
    diff verify_out/pred_obs_nonpreemptive_1shot.01 verify_out/pred_obs_preemptive_decode_1shot.01 | head -20
fi
