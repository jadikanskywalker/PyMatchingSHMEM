...
#SBATCH --output=/mnt/DISCL/home/jadhicks/PyMatchingSHMEM/verify_out/verify_obs_preemptive_overlap_3way_mixed-%j.out
...
python3 - "$DET" "$DEM" verify_out/det_zero_3way_mixed.b8 <<'EOF'
import sys
import stim
...
oshrun -n 2 build_sos/pymatching predict --dem "$DEM" \
  --in verify_out/det_zero_3way_mixed.b8 --in_format b8 \
  --out verify_out/pred_obs_preemptive_3way_mixed.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 10 \
  --extract_preemptively \
  > verify_out/verify_obs_preemptive_3way_mixed.stdout 2>&1
echo "exit=$?" >> verify_out/verify_obs_preemptive_3way_mixed.stdout
