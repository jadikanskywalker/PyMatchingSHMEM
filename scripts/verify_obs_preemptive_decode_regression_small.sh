...
#SBATCH --output=/mnt/DISCL/home/jadhicks/PyMatchingSHMEM/verify_out/verify_obs_preemptive_decode_regression_small-%j.out
...
python3 - "$DET" "$DEM" verify_out/det_small20.b8 <<'EOF'
import sys
import stim
...
stim.write_shot_data_file(data=data[:20], path=out_path, format="b8", num_detectors=num_dets)
EOF
...
oshrun -n 2 build_sos/pymatching predict --dem "$DEM" \
  --in verify_out/det_small20.b8 --in_format b8 \
  --out verify_out/pred_obs_nonpreemptive_small.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 1 \
  > verify_out/verify_obs_preemptive_decode_regression_small_nonpreemptive.stdout 2>&1
echo "exit=$?" >> verify_out/verify_obs_preemptive_decode_regression_small_nonpreemptive.stdout

echo "Preemptive OBS (Phase 2 under test)..."
oshrun -n 2 build_sos/pymatching predict --dem "$DEM" \
  --in verify_out/det_small20.b8 --in_format b8 \
  --out verify_out/pred_obs_preemptive_decode_small.01 --out_format 01 \
  --task_division_strategy observable \
  --extraction_unit_size 1 \
  --extract_preemptively \
  > verify_out/verify_obs_preemptive_decode_regression_small_preemptive.stdout 2>&1
echo "exit=$?" >> verify_out/verify_obs_preemptive_decode_regression_small_preemptive.stdout

echo "Diffing predictions..."
if diff -q verify_out/pred_obs_nonpreemptive_small.01 verify_out/pred_obs_preemptive_decode_small.01 > /dev/null; then
    ...
    diff verify_out/pred_obs_nonpreemptive_small.01 verify_out/pred_obs_preemptive_decode_small.01 | head -20
fi
