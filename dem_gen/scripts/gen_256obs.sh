#!/bin/bash
#SBATCH --job-name=gen_256obs
#SBATCH --output=dem_gen/out/gen_256obs-%j.out
...
# See gen_72obs.sh for the --graph_cache_path/--gen_rounds rationale. preset_256obs()'s own
# exact schedule length is 1537 (two independent preset_128obs() copies ending at round 1472,
# +2*M idle gap, +one plain top-level join gate -- continues preset_64obs's sparse top-join
# style one level higher). --gen_rounds must be an exact multiple of --rounds_per_partition
# (see project_dem_presets memory) -- 1537 isn't a multiple of 21, so this rounds up to 1554
# (=21*74), adding a few harmless idle rounds after the last real gate. ~218GB transient DEM
# (empirical ~555KB/(obs*round) scaling from the 36obs/64obs DEMs) -- larger partition count
# (18944 vs 9088) than gen_128obs.sh.
